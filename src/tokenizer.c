...
char** tokenizer(char* query_string){
    ...
    char** token_array = (char**)malloc(sizeof(char*)*initial_tokens_array_size);
    ...
    for(int i=0; toggle_tokenizing; i++){
        ...
        token_array[i] = (char*)malloc(sizeof(char)*50);
        ...
        if(i < double_tokens_array_size*initial_tokens_array_size){
            while(!next_token_toggle){
                if (current_token_char_index < 50*double_token_size){
                    if(get_previous_token){
                        token_array[i][current_token_char_index] = query_string[current_query_string_index-1];
                        token_array[i][current_token_char_index+1] = '\0';
                            ...
                            case 32:
                                if(!escape_toggle){
                                    if(current_token_char_index)
                                        token_array[i][current_token_char_index] = '\0';
                                    else
                                        --i;
                                    next_token_toggle = 1;
                                }
                                else {
                                    token_array[i][current_token_char_index] = query_string[current_query_string_index];
                                }
                                break;
                            case '\0':
                                token_array[i][current_token_char_index] = '\0';
                    ...
                    for (int j=0; j<initial_token_array_size*double_token_size/2; j++){
                        new_token_array[j] = token_array[i][j];
                    }
                    free(token_array[i]);
                    token_array[i] = new_token_array;
                }
            }
        }
        else{
            // Double the tokens array size and copy it over.
                ...
                for (int i=0; i<initial_tokens_array_size*double_token_size/2; i++){
                    new_tokens_array[i] = token_array[i];
                }
                free(token_array);
                token_array = new_tokens_array;
        }
    }
    return token_array;
}
